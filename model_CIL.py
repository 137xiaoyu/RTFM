...
class Att_MMIL(nn.Module):
    ...
    def forward(self, x, seq_len):
        ...
        v_logits = torch.sigmoid(frame_prob)  # (b, n, 1)
        mmil_logits = self.clas(v_logits, seq_len)  # (b, 1)
        return mmil_logits, v_logits
...
class Model(nn.Module):
    def __init__(self, n_features, batch_size):
        ...
        ffn_dim = 256
        dropout = 0.1
        self.fc_v = nn.Linear(n_features, hid_dim)
        ...
        self.att_mmil = Att_MMIL(hid_dim, 1)
    ...
    def forward(self, inputs, seq_len):
        ...
        mmil_logits, visual_logits = self.att_mmil(out, seq_len)

        # out = self.Aggregate(out)

        # out = self.drop_out(out)

        return mmil_logits, visual_logits, out
